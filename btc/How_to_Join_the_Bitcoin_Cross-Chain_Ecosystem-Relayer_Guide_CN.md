...
在跨链生态中，Relayer起到了举足轻重的作用，它会转发跨链交易和区块头，实现Poly与外界的信息交互，可以说是生态的对外沟通的媒介。
...
比特币Relayer实现了比特币网络的监听，能识别并转发跨链交易，向Poly提交区块头，并获取收益，同时监听Poly，广播Poly构造的跨链交易，只要有一个Relayer在工作，那么整个比特币跨链生态就可以持续运作。

- **先决条件**：部署Relayer需要提前部署好比特币的[全节点](https://bitcoin.org/en/download)，Relayer需要使用全节点的数据，全节点同步好比特币账本，然后再启动Relayer；
- **交易转发**：Relayer会持续扫描每个比特币区块，找到跨链交易并从全节点获取交易的梅克尔证明，一同提交给Poly，对还没有足够确认数的交易，Relayer会存储下来，等到Poly维护的比特币区块头达到一定高度，有足够的确认后，再发送给Poly；
- **区块头同步**：Relayer负责提交比特币区块头到Poly，能处理比特币常见的分叉情况，稳定并正确地向Poly提交区块头；
- **广播交易**：Poly会构造比特币的返回交易，Relayer会把交易广播到比特币网络中。

## 准备

### 1. 申请联盟链钱包

申请一个Poly的钱包，如果有本体钱包直接使用即可，二者钱包是通用的。
...
```json
{
  ...
  "allia_ob_conf": {
    "alliance_json_rpc_address": "http://ip:40336", //要连接的Poly地址*
    "allia_ob_loop_wait_time": 3, //监听Poly的时间间隔
    "watching_key": "btcTxToRelay", //监听Poly的事件关键词
    "wallet_file": "relayer_btc/wallet.dat", //你Poly钱包的路径*
